...
const Complaints = () => {
  ...
  const handleAssignSubmit = async (e) => {
    ...
    try {
      const response = await axios.put(
        endpoint + `assign-complaint/${assign.staff}/`,
        assign,
        {
          headers: {
            Authorization: `Bearer ${accessToken}`,
  ...
  useEffect(() => {
    axios
      .get(endpoint + "complaints-by-user/", {
        headers: {
          Authorization: `Bearer ${accessToken}`,
        },
      })
  ...
  useEffect(() => {
    axios
      .get(endpoint + "assigned-complaints/", {
        headers: {
          Authorization: `Bearer ${accessToken}`,
        },
      })
  ...
  useEffect(() => {
    axios
      .get(endpoint + "assign-complaint/staff_list/", {
        headers: {
          Authorization: `Bearer ${accessToken}`,
        },
      })
        ...
        <div className="grid gap-y-10">
          {userType != "1" && userType != "2" ? (
            <div className={`${card}`}>
              ...
              <p>
                Dear valued users, Your feedback is crucial to us! If you have
                any concerns, complaints, or suggestions regarding our services,
                we encourage you to share them with us. Your input helps us
                improve and ensures that we continue to meet your needs
                effectively.
              </p>
              <button onClick={openModal} className={`${button} bg-blue-600`}>
                Submit a Complaint
              </button>
            </div>
          ) : (
                ...
                <div className={`${card}`}>
                  ...
                  <div className="grid lg:grid-cols-2 gap-8">
                    {allComplaints.map((x, key) => (
                      <div
                        className={`rounded-md border p-4 w-full bg-gray-100`}
                        key={key}
                      >
                        <div>
                          <p className="text-right mb-4 text-sm">
                            {moment(x.submission_date).format(
                              "MMMM Do YYYY, h:mm:ss a"
                            )}
                          </p>
                          <p className="title font-semibold text-lg">
                            {x.title}
                          </p>
                          ...
                          <div className="grid grid-cols-2">
                            ...
                            <p className="justify-self-end bg-white self-end rounded-xl w-min px-4 py-1">
                              {x.category}
                            </p>
                          </div>
                          <button
          ...
          >
            <button onClick={closeModal} className="float-right">
              <IoClose size={20} className="text-blue-600 mb-4" />
            </button>
            <div className="shadow-sm clear-both p-4">
              ...
              <form action="" className="grid gap-y-4" onSubmit={handleSubmit}>
                ...
                <textarea
                  ...
                  className="rounded w-full mt-1 p-2 border border-blue-400"
                  onChange={handleChange}
                ></textarea>
                ...
                <button type="submit" className={`${button} bg-green-600`}>
                  Submit
                </button>
              </form>
            </div>
          ...
          >
            <button onClick={closeFeedbackModal} className="float-right">
              <IoClose size={20} className="text-blue-600 mb-4" />
            </button>
            <div className="shadow-sm clear-both p-4">
          ...
          >
            <button onClick={closeAssignModal} className="float-right">
              <IoClose size={20} className="text-blue-600 mb-4" />
            </button>
            <div className="shadow-sm clear-both p-4">
